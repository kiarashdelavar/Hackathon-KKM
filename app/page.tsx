...
const initialChats: Chat[] = [
  { id: 1, title: "Amsterdam Groceries Arbitrage", isPinned: true },
  { id: 2, title: "Llama-3 Integration Docs", isPinned: false },
  { id: 3, title: "VAT Exemption Report F-01", isPinned: false },
];
...
const initialMessages: Message[] = [
  {
    id: 1,
    role: "user",
    text: "سلام عباس آقا! میتونی تو خرید مک‌بوک با بهترین قیمت و صفر درصد مالیات کمکم کنی؟",
  },
  {
    id: 2,
    role: "ai",
    text: "Understood! Initiating a Real-time Arbitrage Scan. I will check the official prices on Apple Stores in NL, DE, FR, UK, and CH.\n\nAnalysis underway...\nI'll also generate a virtual b-u-n-q card for the best-priced currency automatically. For your taxes, I'll parse the receipt and create a tax report for your accountant. Hang tight.",
  },
];
...
export default function Home() {
  ...
  const sendToBackend = async (payloadMsgs: Message[]) => {
    ...
    const payload: ClaudePayload = {
      ...
      userContext: {
        accountId: "mock_user_123",
        name: `${mockUser.firstName} ${mockUser.lastName}`,
      },
      messages: payloadMsgs.map((m) => ({
        role: m.role,
        content: m.text,
        attachments: [
          ...(m.fileName ? [{ type: "file", name: m.fileName }] : []),
          ...(m.audioUrl ? [{ type: "audio", url: m.audioUrl }] : []),
        ],
      })),
    };
    ...
    try {
      console.log("Sending to Backend API (Claude):", payload);
      await new Promise((resolve) => setTimeout(resolve, 1500));
      const mockClaudeResponse = "Got it! I'm analyzing the data right now.";
      setMessages((prev) =>
        prev.map((m) =>
          m.id === tempAiId
            ? { ...m, text: mockClaudeResponse, isLoading: false }
            : m,
        ),
      );
    } catch (error) {
